          ...
          <div className="mt-3 space-y-2">
            {selectedRole.workflow.map((step, index) => (
              <div className="flex items-start gap-2 text-sm text-slate-600" key={step}>
                <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded bg-white text-xs font-semibold text-workday-blue shadow-sm ring-1 ring-slate-200">
                  {index + 1}
                </span>
                <span>{step}</span>
              </div>
        ...
        <div className="dashboard-panel-muted p-4">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {selectedRole.primaryTabs.map((tab) => (
              <button
                className="inline-flex h-8 items-center gap-1.5 rounded-md bg-white px-2.5 text-xs font-semibold text-slate-700 shadow-sm ring-1 ring-slate-200 transition hover:bg-blue-50 hover:text-workday-blue focus:outline-none focus:ring-2 focus:ring-workday-blue"
                key={tab.tabId}
        ...
        <div>
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {selectedRole.proofPoints.map((point) => (
              <span
                className="rounded-md border border-slate-200 bg-white px-3 py-1 text-xs font-semibold text-slate-700 shadow-sm"
                key={point}
              >
                {point}
              </span>
